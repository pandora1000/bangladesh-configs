...
window.addEventListener("DOMContentLoaded", (() => {
  ...
  for (const link of Array.from(optionsLinks)) link.addEventListener("click", (event => {
  ...
  // Translate keys for Mac users
    ...
    for (const elem of Array.from(kbdElems)) if (elem.dataset.mac) elem.textContent = elem.dataset.mac;
  }
}));
